"""A bidirectional LSTM model with binary labels (toxic or non-toxic)"""
...
def get_model(embedding_matrix: np.ndarray = None,
              ...
              dropout: float = config.DROPOUT_BL,
              num_lstm_units: int = config.NUM_LSTM_UNITS_BL,
              num_dense_units: int = config.NUM_DENSE_UNITS_BL,
              learning_rate: float = config.LEARNING_RATE_BL):
...
checkpoint = ModelCheckpoint(config.TRAINED_MODEL_DIR + "/checkpoint_BL.h5", 
                             monitor="val_loss", 
                             verbose=1, 
                             save_best_only=True, 
                             mode="min")
